...
public class DayAavAnalysePolicy extends ChoosePolicy {
    ...
    public DayAavAnalysePolicy(int period_buy, int period_sell, boolean isReferenceSz, int buyCondition, int sellCondition) {
        ...
        this.period_buy = period_buy;
        this.period_sell = period_sell;
        periodStr_buy = "getPrice_" + period_buy + "day";
        periodStr_sell = "getPrice_" + period_sell + "day";
    ...
    int period_buy, period_sell;

   
    public static ChoosePolicy getInstance(String fullargs) {
        String[] args = fullargs.split(",");
        int period_buy = Integer.parseInt(args[0]);
        int period_sell = Integer.parseInt(args[1]);
        boolean isReferenceSz = Boolean.parseBoolean(args[2]);
        int buyCondition = Integer.parseInt(args[3]);
        int sellCondition = Integer.parseInt(args[4]);
        return new DayAavAnalysePolicy(period_buy, period_sell, isReferenceSz, buyCondition, sellCondition);
    }
    ...
    private boolean canbuy_3(Sto sto, Calendar nextChargeDay) {//5avg 突破10avg buy
        ...
        if (lastDayPrice_5AVGday < lastDayPrice_10AVGday) {
            return false;
        }
        ...
        if (lastDayPrice_5AVGday_1 < lastDayPrice_10AVGday_1) {
            return true;
        }
    ...
    public String getFullargs() {
        return this.period_buy + "," + this.period_sell + "," + this.isReferenceSz + "," + this.buyCondition + "," + this.sellCondition;
        //return period_buy+","+period_sell;
    }
}
