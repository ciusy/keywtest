...
public class CapitalSituation {
	...
	public Map<String, ShareHolding> getHoldingMap() {
		Map<String, ShareHolding> holdingMap = new HashMap<String, ShareHolding>();
	...
	public int increaseSto(Sto sto, float plan_price, int operateAmount,
			Date date) {
		if (this.leftMoney
				.compareTo(new BigDecimal(operateAmount * plan_price)) > 0) {
			this.leftMoney = this.leftMoney.subtract(new BigDecimal(
					operateAmount * plan_price));
			...
			if (isAlreadyOwn) {
				...
				float costPrice = (preCostPrice * alreadyMount + operateAmount
						* plan_price)
						/ (nowMount * 1.0f);
			...
			float brokerage = operateAmount * plan_price
					* (ChargeCenter.brokerageRate + ChargeCenter.stampRate);
			if (brokerage < ChargeCenter.minimumBrokerage)
	...
	public int reduceSto(Sto sto, float plan_price, int operateAmount, Date date) {
		...
		if (flag_isHodingThisSto) {
			...
			if (nowMount == 0) {
				...
				shtemp_SellAll.setCostPrice((shtemp_SellAll.getCostPrice()
						* alreadyMount - plan_price * operateAmount)
						/ (nowMount * 1.0f));
				shtemp_SellAll.availableAmountForSell -= operateAmount;
				shtemp_SellAll.hodingAmount = nowMount;
			}
			this.leftMoney = this.leftMoney.add(new BigDecimal(operateAmount
					* plan_price));
			float brokerage = operateAmount * plan_price
					* (ChargeCenter.brokerageRate + ChargeCenter.stampRate);
			if (brokerage < ChargeCenter.minimumBrokerage)
