...
public class CapitalSituation {
    ...
    public int increaseSto(Sto sto, float plan_price, int operateAmount,Date date) {
        if (this.leftMoney.compareTo(new BigDecimal(operateAmount * plan_price)) > 0) {
            ...
            float brokerage=operateAmount * plan_price*(ChargeCenter.brokerageRate+ChargeCenter.stampRate);
            if(brokerage<ChargeCenter.minimumBrokerage)brokerage=ChargeCenter.minimumBrokerage;
            ...
            cfDao.add(new CapitalFlow(getOwnerPerson().getUserUuid(), sto.getName(), CapitalFlow.sto_buy, date, plan_price, operateAmount, new BigDecimal(plan_price*operateAmount), leftMoney, new BigDecimal(brokerage), sto.getCode()));
    ...
    public int reduceSto(Sto sto, float plan_price, int operateAmount,Date date) {
        ...
        if (flag_isHodingThisSto) {
            ...
            logger.info("已经卖出股票" + sto.getName() + operateAmount + "股，成交价格为" + plan_price + "元");
             cfDao.add(new CapitalFlow(getOwnerPerson().getUserUuid(), sto.getName(), CapitalFlow.sto_sell, date, plan_price, operateAmount, new BigDecimal(plan_price*operateAmount), leftMoney, new BigDecimal(brokerage), sto.getCode()));
            return ChargeCenter.charge_succuss;
        } else {
